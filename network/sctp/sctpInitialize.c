 ...
 */
struct sctp *sctpInitialize(int localpt, struct netaddr *localaddr,
                            int naddrs)
{
    ...
    for (i = 0; i < naddrs; i++)
    {
        memcpy(&instance->localip[i], &localaddr[i],
               sizeof(localaddr[i]));
    }
    ...
    SCTP_TRACE("SCTP instance initialized with %d home(s)", naddrs);
...
static int sctpGetPort(void)
{
    ...
    for (i = 0; i < NSCTP; i++)
    {
        ...
        {
            nextport =
                (nextport + 1) % (SCTP_HIGHPORT - SCTP_LOWPORT + 1);
            i = 0;
        }
    }
